...
if(user.modifiedCount){
    ...
    return next(new Error('not registerd account',{cause:400}))
}
...
export const NewconfirmEmail = async(req,res,next)=>{
                        ...
                        <table border="0" cellpadding="0" cellspacing="0">
                          <tr>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0">
                              <tr>
                                <td align="center" bgcolor="#1a82e2" style="border-radius: 6px;">
                                  <a href="${link}" target="_blank" style="display: inline-block; padding: 16px 36px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; color: #ffffff; text-decoration: none; border-radius: 6px;">verfiy email</a>
                                </td>
                          </tr>
                        </table>
                      </td>
    ...
    await sendEmail(email,`confirm email`,html)
    return res.status(200).send(` <p> check your email box a new email sent successfuly</p>`)
}
export const login = async(req,res,next)=>{
        ...
        if(!user){
            ...
            if(!match){
                ...
                return res.status(200).json({message:"Done",token,Rtoken});
            }
    }
}
export const sendCode = asyncHandler(async(req,res,next)=>{
  const {email}=req.body
  let code= customAlphabet('123456789',4)
  code=code();
  const user = await userModel.findOneAndUpdate({email},{forgetCode:code},{new:true})
  const html=`<p>your code is ${code}</p>`
  await sendEmail(email,`forget password code`,html)
